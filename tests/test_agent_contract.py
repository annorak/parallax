...
def library(monkeypatch):
    fake = SimpleNamespace(
        Browser=Mock(return_value=SimpleNamespace(kill=AsyncMock())),
        Agent=Mock(return_value=SimpleNamespace(run=AsyncMock())),
        ChatOpenAI=Mock(),
        ChatAnthropic=Mock(),
    )
...
def test_browser_can_contact_only_fake_site(tmp_path, monkeypatch):
    ...
    async def run(max_steps):
        ...
        result = await session.cdp_client.send.Page.navigate(
            {"url": forbidden}, session_id=session.session_id,
        )
